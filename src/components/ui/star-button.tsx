...
export function StarButton({
  ...
  return (
    <div
      ...
      ref={pathRef}
      className={cn(
        "relative z-[3] overflow-hidden h-14 px-8 py-3 inline-flex items-center justify-center gap-3 whitespace-nowrap rounded-full text-base font-semibold tracking-wider transition-all duration-300 select-none shadow-lg cursor-pointer group/star-button hover:scale-[1.03] hover:-translate-y-[3px] active:scale-[0.98]",
        className
      )}
      {...props}
    >
      ...
      />
      <div
        className="absolute inset-0 border-white/20 z-[4] overflow-hidden rounded-[inherit] text-foreground"
        style={{ borderWidth: "var(--border-width)" }}
        aria-hidden="true"
      >
        <StarBackground color={backgroundColor} />
      </div>
      <span className="z-10 relative text-white font-bold flex items-center gap-2 drop-shadow-sm">
        {children}
      </span>
    </div>
  );
